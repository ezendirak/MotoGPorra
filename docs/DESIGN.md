...
- **⚠️ El CLI de Supabase está enlazado al proyecto de PRUEBAS** (`rczchxjrmybveasydymh`), el que se creó para validar las ligas. Mientras siga así, `npm run db:push` y `gen types --linked` van ahí y no a la porra real. Se devuelve con `npx supabase link --project-ref vjttgjjmdsnlurwlcmgb`. Las claves de ese proyecto están en `.env.pruebas`, en la raíz, ignorado por git.
