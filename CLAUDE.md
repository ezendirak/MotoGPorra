...
Porra anual de MotoGP. Cada participante predice el podio (🥇🥈🥉) de cada Gran Premio antes de que empiece la calificación. El resultado oficial se importa solo y las puntuaciones se recalculan sin intervención manual.
...
**Las apuestas cierran 5 minutos antes de la Q1**, no antes de FP1: se apuesta con los entrenamientos vistos y sin la parrilla de salida. Lo calcula `mappers.betting_close_time`, y si un fin de semana no tuviera Q1 el respaldo cierra en la primera sesión — antes de lo previsto, nunca después.
...
npm run db:verify    # 38 pruebas de esquema, RLS y clasificación contra la base real
...
cd apps/sync && ../../.venv/Scripts/python -m pytest -q
```
...
- **La cuenta atrás no puede leer el reloj del dispositivo.** Comparar `closes_at` con `new Date()` del navegador hacía que un móvil con la hora atrasada anunciara «Cierra en 6 min» con la Q1 ya rodando, y además no disparaba el `router.refresh()` que retira el formulario. El servidor manda los milisegundos restantes (`msHasta`) y el cliente descuenta con `performance.now()`, que es monótono. Ninguna apuesta llegó a colarse —`place_bet` valida con el reloj del servidor—, pero la aplicación le estaba diciendo a esa persona que aún podía apostar.
- **Al elegir podio, un piloto ya colocado no se bloquea: se intercambia.** `colocarPiloto` en `utils/podium.ts`. Deshabilitarlo obligaba a quitarlo de su posición y buscarlo otra vez para subirlo, que es justo el gesto más frecuente.
